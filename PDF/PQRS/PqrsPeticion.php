...
function Header()
{
    ...
    $this->SetLineWidth(.3);
    $this->SetTitle('PeticionesDelMes');
    $this->Cell(30,10,'Peticiones',);
    // Salto de línea
    $this->Ln(20);
...
$Consulta = "SELECT * FROM pqrs INNER JOIN usuarios ON pqrs.Documento=usuarios.Documento INNER JOIN estadopqrs ON pqrs.ID_Estadopqrs=estadopqrs.ID_Estadopqrs WHERE ID_Tipopqrs=1 AND DATE_FORMAT(Fecha_pqrs, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m');";
