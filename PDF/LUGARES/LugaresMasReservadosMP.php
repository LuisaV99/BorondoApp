...
$Consulta = "SELECT lugares.*, COUNT(reservas.ID_Lugar) as NReservas FROM lugares INNER JOIN reservas on lugares.ID_Lugar=reservas.ID_Lugar WHERE PERIOD_DIFF(DATE_FORMAT(NOW(),'%Y%m'),DATE_FORMAT(reservas.FechaRgR,'%Y%m'))=1 ORDER BY NReservas DESC";
